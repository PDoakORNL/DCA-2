...
void DcaData<Parameters, DT>::write(Writer& writer) {
  ...
  if (debug_write_samples) {
    constexpr int constrained_tuples[12][4] = {
        {0, 0, 0, 2}, {0, 0, 2, 0}, {0, 0, 2, 2}, {0, 2, 0, 0},
        {0, 2, 0, 2}, {0, 2, 2, 0}, {2, 0, 0, 0}, {2, 0, 0, 2},
        {2, 0, 2, 0}, {2, 2, 0, 0}, {2, 2, 0, 2}, {2, 2, 2, 0},
    };
    ...
    std::cout << "  constrained samples using 12 hard-coded tuples with"
              << " (b1!=b3) or (b2!=b4):\n";
      ...
      for (const auto& G4_channel : G4_) {
        if (debug_write_samples) {
          ...
          std::cout << "  " << G4_channel.get_name() << " samples (fixed k1="
                    ...
                    << ", wex=" << fixed_g4_wex
                    << ", 12 hard-coded tuples with (b1!=b3) or (b2!=b4)):\n";
          int printed_g4 = 0;
          for (int it = 0; it < 12; ++it) {
            const int b1 = constrained_tuples[it][0];
            const int b2 = constrained_tuples[it][1];
