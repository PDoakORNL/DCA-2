...
//Total frequencies are (-n,n) n_frqs with 0 missing.
...
TYPED_TEST(CachedNdftGpuTest, Execute) {
  ...
  // f_baseline_.
  const double time =
    computeWithFastNDFT<Scalar>(TestFixture::configuration_, TestFixture::M_, f_w_fast);

  auto f_baseline = TestFixture::compute2DFTBaseline();
  const auto err = dca::func::util::difference(f_baseline, f_w_fast);
  EXPECT_LT(err.l_inf, 100 * std::numeric_limits<dca::util::RealAlias<Scalar>>::epsilon());
  
  std::cout << "\nCached GPU ndft time [sec]:\t " << time << "\n";
}

template <typename Scalar>
double computeWithFastNDFT(const typename CachedNdftGpuTest<Scalar>::Configuration& config,
                           const typename CachedNdftGpuTest<Scalar>::Matrix& M,
                           typename CachedNdftGpuTest<Scalar>::F_w_w& f_w) {
  using Complex = dca::util::ComplexAlias<Scalar>;
  dca::linalg::util::initializeMagma();
  dca::linalg::util::MagmaQueue queue;

  using BDmn = typename CachedNdftGpuTest<Scalar>::BDmn;
  using RDmn = typename CachedNdftGpuTest<Scalar>::RDmn;
  using FreqDmn = typename CachedNdftGpuTest<Scalar>::FreqDmn;

  using NftType =
    dca::phys::solver::accumulator::CachedNdft<Scalar, RDmn, FreqDmn, FreqDmn, dca::linalg::GPU>;
  NftType nft_obj(queue);
  EXPECT_EQ(magma_queue_get_cuda_stream(queue), nft_obj.get_stream());

  dca::linalg::Matrix<Scalar, dca::linalg::GPU> M_dev(M);
      ...
      for (int r2 = 0; r2 < nr; ++r2)
        for (int r1 = 0; r1 < nr; ++r1)
          for (int w2 = 0; w2 < FreqDmn::dmn_size(); ++w2)
            for (int w1 = 0; w1 < FreqDmn::dmn_size(); ++w1) {
