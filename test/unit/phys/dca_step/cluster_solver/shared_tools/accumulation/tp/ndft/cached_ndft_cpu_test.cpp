...
TYPED_TEST(CachedNdftCpuTest, Execute) {
  ...
  EXPECT_LT(err.l_inf, 100 * std::numeric_limits<dca::util::RealAlias<Real>>::epsilon());
...
double computeWithFastDNFT(const typename CachedNdftCpuTest<Real>::Configuration& config,
  ...
  dca::func::function<dca::util::ComplexAlias<Real>,
                      dca::func::dmn_variadic<BDmn, BDmn, RDmn, RDmn, FreqDmn, FreqDmn>>
      f_b_b_r_r_w_w;
  dca::phys::solver::accumulator::CachedNdft<Real, RDmn, FreqDmn, FreqDmn, dca::linalg::CPU> nft_obj;
        ...
        for (int r1 = 0; r1 < RDmn::dmn_size(); ++r1)
          for (int w2 = 0; w2 < FreqDmn::dmn_size(); ++w2)
            for (int w1 = 0; w1 < FreqDmn::dmn_size(); ++w1) {
              f_w(b1, b2, r1, r2, w1 , w2) = f_b_b_r_r_w_w(b1, b2, r1, r2, w1, w2);
            }
